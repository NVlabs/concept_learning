...
import torch
import copy
from storm_kit.mpc.rollout.se3_rollout import SE3Rollout
...
class SE3OptimizationRollout(SE3Rollout):
    def __init__(self, exp_params, world_params=None,
                 tensor_args={'device':'cpu','dtype':torch.float32}):
        super().__init__(exp_params, world_params, tensor_args)
    ...
    def cost_fn(self, state_dict, action_batch, no_coll=False, horizon_cost=True, return_dist=False):
        if return_dist:
            cost, rot_err_norm, goal_dist = super(SE3OptimizationRollout, self).cost_fn(state_dict, action_batch, no_coll, horizon_cost, return_dist)
        else:
            cost = super(SE3OptimizationRollout, self).cost_fn(state_dict, action_batch, no_coll, horizon_cost, return_dist)
    ...
    def update_params(self, goal_state=None, goal_ee_pos=None, goal_ee_rot=None, goal_ee_quat=None,
        ...
        pc_seg: N_points
        """
        super(SE3OptimizationRollout, self).update_params(goal_state, goal_ee_pos, goal_ee_rot, goal_ee_quat)
        if (pc is not None):
